...
def process(image):
    ...
    #image_YYY = cv2.merge([image_Y,image_Y,image_Y])
    image_RGB = cv2.cvtColor(image,cv2.COLOR_BGR2RGB)
    result = Ying_2017_CAIP(image_RGB)
    result = cv2.cvtColor(result,cv2.COLOR_RGB2BGR)
    #result = cv2.merge([result[:,:,0],image_U,image_V])
    #result = cv2.cvtColor(result,cv2.COLOR_YUV2BGR)
    return result
    #cv2.imwrite(os.path.splitext(sys.argv[1])[0]+'_ying.jpg',result)
    #plt.imshow(result)
    #plt.show()
